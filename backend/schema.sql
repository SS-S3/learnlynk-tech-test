...
create index idx_leads_tenant_owner_stage_created on public.leads (tenant_id, owner_id, stage, created_at desc);
...
create index idx_applications_tenant_lead_stage on public.applications (tenant_id, lead_id, stage);
...
alter table public.tasks add constraint chk_tasks_type check (type in ('call', 'email', 'review'));
alter table public.tasks add constraint chk_tasks_due_at check (due_at >= created_at);
